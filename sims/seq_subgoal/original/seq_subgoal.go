...
var ParamSets = params.Sets{
	{Name: "Base", Desc: "these are the best params", Sheets: params.Sheets{
		"Network": &params.Sheet{
			...
			{Sel: "Layer", Desc: "needs some special inhibition and learning params",
				Params: params.Params{
					"Layer.Learn.AvgL.Gain":    "3", // this is critical! 2.5 def doesn't work
					"Layer.Inhib.Layer.Gi":     "2.5",
					"Layer.Inhib.ActAvg.Init":  "0.2",
					"Layer.Inhib.ActAvg.Fixed": "true",
					"Layer.Act.Gbar.L":         "0.3",
				}},
			{Sel: ".PPC", Desc: "needs some special inhibition and learning params",
				Params: params.Params{
					"Layer.Learn.AvgL.Gain":    "3", // this is critical! 2.5 def doesn't work
			...
			{Sel: ".ATL", Desc: "needs some special inhibition and learning params",
				Params: params.Params{
					"Layer.Learn.AvgL.Gain":    "3", // this is critical! 2.5 def doesn't work
	...
	{Name: "Hebbian", Desc: "Hebb-only learning params", Sheets: params.Sheets{
		"Network": &params.Sheet{
			{Sel: "Prjn", Desc: "",
				Params: params.Params{
...
type Sim struct {
	...
	LineMaze     *etable.Table     `view:"no-inline" desc:"only train on a straight line`
	OpenField    *etable.Table     `view:"no-inline" desc:"train all adjacent grid transitions`
...
func (ss *Sim) New() {
	...
	ss.Pats = LineMaze
	ss.LineMaze = &etable.Table{}
	ss.OpenField = &etable.Table{}
	...
	ss.TestInterval = 5
	ss.LayStatNms = []string{"ActualState_Input", "ActualAction_Output"}
}
...
func (ss *Sim) ConfigEnv() {
	...
	ss.TrainEnv.Table = etable.NewIdxView(ss.LineMaze)
	ss.TrainEnv.Validate()
	ss.TrainEnv.Run.Max = ss.MaxRuns // note: we are not setting epoch max -- do that manually

	ss.TestEnv.Nm = "TestEnv"
	ss.TestEnv.Dsc = "testing params and state"
	ss.TestEnv.Table = etable.NewIdxView(ss.LineMaze)
...
func (ss *Sim) UpdateEnv() {
	...
	case LineMaze:
		...
		ss.TestEnv.Table = etable.NewIdxView(ss.LineMaze)
	case OpenField:
		ss.TrainEnv.Table = etable.NewIdxView(ss.OpenField)
		ss.TestEnv.Table = etable.NewIdxView(ss.OpenField)
...
func (ss *Sim) ConfigNet(net *leabra.Network) {
	...
	curhid.SetClass("PPC")
	goalhid.SetClass("PPC")
	actrelhid.SetClass("PPC")

	inhid := net.ConnectLayers(curin, statenodes, prjn.NewOneToOne(), emer.Forward)
	net.ConnectLayers(goalin, statenodes, prjn.NewOneToOne(), emer.Forward)
	net.ConnectLayers(actout, actnodes, prjn.NewOneToOne(), emer.Forward)

	net.ConnectLayers(statenodes, statenodes, prjn.NewFull(), emer.Lateral)
	net.ConnectLayers(actnodes, actnodes, prjn.NewFull(), emer.Lateral)
	net.ConnectLayers(statenodes, actnodes, prjn.NewFull(), emer.Forward)
	net.ConnectLayers(actnodes, statenodes, prjn.NewFull(), emer.Forward)
	...
	net.ConnectLayers(actbtwn, actout, prjn.NewOneToOne(), emer.Forward)
	net.ConnectLayers(actout, actbtwn, prjn.NewOneToOne(), emer.Back)

	inhid.SetClass("InputToHidden")

	goalin.SetRelPos(relpos.Rel{Rel: relpos.RightOf, Other: "ActualAction_Output", YAlign: relpos.Front, XAlign: relpos.Left})
	actout.SetRelPos(relpos.Rel{Rel: relpos.RightOf, Other: "ActualState_Input", YAlign: relpos.Front, XAlign: relpos.Left})
	statenodes.SetRelPos(relpos.Rel{Rel: relpos.Above, Other: "ActualState_Input", YAlign: relpos.Front, XAlign: relpos.Left})
	actnodes.SetRelPos(relpos.Rel{Rel: relpos.Above, Other: "StateNodes", YAlign: relpos.Front, XAlign: relpos.Left})
	curhid.SetRelPos(relpos.Rel{Rel: relpos.Above, Other: "Goal_Input", YAlign: relpos.Front, XAlign: relpos.Left})
	actrelhid.SetRelPos(relpos.Rel{Rel: relpos.RightOf, Other: "CurHidden", YAlign: relpos.Front, XAlign: relpos.Left})
	actbtwn.SetRelPos(relpos.Rel{Rel: relpos.Above, Other: "ActRelHid", YAlign: relpos.Front, XAlign: relpos.Left})
	goalhid.SetRelPos(relpos.Rel{Rel: relpos.RightOf, Other: "ActRelHid", YAlign: relpos.Front, XAlign: relpos.Left})
...
func (ss *Sim) TrainTrial() {
	...
	ss.Net.LayerByName("ActualAction_Output").SetType(emer.Target)
...
func (ss *Sim) TrialStats(accum bool) (sse, avgsse, cosdiff float64) {
	out := ss.Net.LayerByName("ActualAction_Output").(leabra.LeabraLayer).AsLeabra()
...
func (ss *Sim) TestTrial(returnOnChg bool) {
	...
	// note: type must be in place before apply inputs
	ss.Net.LayerByName("ActualAction_Output").SetType(emer.Compare)
...
func (ss *Sim) OpenPats() {
	...
	ss.OpenPatAsset(ss.LineMaze, "linemaze.tsv", "LineMaze", "LineMaze Training patterns")
	// err := ss.Easy.OpenCSV("easy.tsv", etable.Tab)
	ss.OpenPatAsset(ss.OpenField, "openfield.tsv", "OpenField", "OpenField Training patterns")
...
func (ss *Sim) LogTstTrl(dt *etable.Table) {
	epc := ss.TrainEnv.Epoch.Prv // this is triggered by increment so use previous value
	inp := ss.Net.LayerByName("ActualState_Input").(leabra.LeabraLayer).AsLeabra()
	out := ss.Net.LayerByName("ActualAction_Output").(leabra.LeabraLayer).AsLeabra()
	...
	ivt := ss.ValsTsr("ActualState_Input")
	ovt := ss.ValsTsr("ActualAction_Output")
...
func (ss *Sim) ConfigGui() *gi.Window {
	...
	gi.SetAppName("SubGoalGen")
	gi.SetAppAbout(`shows how bridging states can be found`)

	win := gi.NewMainWindow("seq_subgoal", "bridging goals with subgoals", width, height)
	ss.Win = win
