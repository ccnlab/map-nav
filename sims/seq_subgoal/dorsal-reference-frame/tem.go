...
func (ss *Sim) New() {
	...
	ss.PosAFNms = []string{"motPPC","pmotPPC","Precuneus"}
}
////////////////////////////////////////////////////////////////////////////////////////////
// 		Configs
...
func  AddConnectedSuperDeep2D(net *deep.Network, name string, shapeY, shapeX int, pulvLay, attn bool) (super, deep, pulv emer.Layer) {
	...
	pj := net.ConnectLayers(deep, pulv, prjn.NewFull(), emer.Forward)
	pj.SetClass("DeepToPulv "+name+"Timescale")

	pj = net.ConnectLayers(pulv, deep, prjn.NewFull(), emer.Forward)
	pj.SetClass("PulvToDeep")
	pj = net.ConnectLayers(pulv, super, prjn.NewFull(), emer.Forward)
	pj.SetClass("PulvToSuper"+name+" Timescale")
...
func (ss *Sim) ConfigNet(net *deep.Network) {
	...
	motppc, _, motppcp := AddConnectedSuperDeep4D(ss.Net, "motPPC",1, 1,worldheight,worldwidth, deep.AddPulv, deep.NoAttnPrjn)
	pmotppc, _, _:= AddConnectedSuperDeep4D(ss.Net, "pmotPPC",1, 1,worldheight,worldwidth, deep.AddPulv, deep.NoAttnPrjn)
	precun, _, precunp:= AddConnectedSuperDeep4D(ss.Net, "Precuneus",1, 1,worldheight,worldwidth, deep.AddPulv, deep.NoAttnPrjn)
	...
	precun.SetRelPos(relpos.Rel{Rel: relpos.RightOf, Other: "pmotPPC", YAlign: relpos.Front, Space: 1})
	...
	pj.SetClass("MotorTRC")
	pj = net.ConnectLayers(va,pmotppc, full, deep.BurstTRC) // act acting like the bg signal from gpi for motor loop
	...
	// input projections
	net.ConnectLayers(in, motppc, onetoone, emer.Forward)
	net.ConnectLayers(in, motppcp, onetoone, emer.Forward)

	// nextpos projections
	net.ConnectLayers(nextpos, precun, onetoone, emer.Forward)
	net.ConnectLayers(nextpos, precunp, onetoone, emer.Forward)
	...
	net.ConnectLayers(pmotppc, upcomingactp, full, emer.Forward)

	// precuneus projections
	ConnectColumnsByName(net, "Precuneus","pmotPPC", full) 
